/* tslint:disable */
/* eslint-disable */
/**
 * World of Tanks
 * OpenAPI specification for the Wargaming.net Public API. The official Wargaming.net Public API documentation can be found at the Wargaming [Developer\'s room](https://developers.wargaming.net/).
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: contact@ace-tanker.net
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface GlobalmapEventaccountratingsItem
 */
export interface GlobalmapEventaccountratingsItem {
    /**
     * Event ID
     * @type {string}
     * @memberof GlobalmapEventaccountratingsItem
     */
    eventId: string;
    /**
     * Player account ID
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    accountId: number;
    /**
     * Clan ID
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    clanId: number;
    /**
     * Clan rating
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    clanRank: number;
    /**
     * Battles fought for current clan
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    battles: number;
    /**
     * Battles to fight in a current clan to get clan award for the season
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    battlesToAward: number;
    /**
     * Award level
     * @type {string}
     * @memberof GlobalmapEventaccountratingsItem
     */
    awardLevel: string;
    /**
     * Date when account data were updated
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    updatedAt: number;
    /**
     * Total Fame Points
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    famePoints: number;
    /**
     * Amount of Fame Points needed to improve personal award
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    famePointsToImproveAward: number;
    /**
     * Front ID
     * @type {string}
     * @memberof GlobalmapEventaccountratingsItem
     */
    frontId: string;
    /**
     * 
     * @type {string}
     * @memberof GlobalmapEventaccountratingsItem
     */
    url: string | null;
    /**
     * Current rating
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    rank: number;
    /**
     * Rating changes during previous turn
     * @type {number}
     * @memberof GlobalmapEventaccountratingsItem
     */
    rankDelta: number;
}

/**
 * Check if a given object implements the GlobalmapEventaccountratingsItem interface.
 */
export function instanceOfGlobalmapEventaccountratingsItem(value: object): value is GlobalmapEventaccountratingsItem {
    if (!('eventId' in value) || value['eventId'] === undefined) return false;
    if (!('accountId' in value) || value['accountId'] === undefined) return false;
    if (!('clanId' in value) || value['clanId'] === undefined) return false;
    if (!('clanRank' in value) || value['clanRank'] === undefined) return false;
    if (!('battles' in value) || value['battles'] === undefined) return false;
    if (!('battlesToAward' in value) || value['battlesToAward'] === undefined) return false;
    if (!('awardLevel' in value) || value['awardLevel'] === undefined) return false;
    if (!('updatedAt' in value) || value['updatedAt'] === undefined) return false;
    if (!('famePoints' in value) || value['famePoints'] === undefined) return false;
    if (!('famePointsToImproveAward' in value) || value['famePointsToImproveAward'] === undefined) return false;
    if (!('frontId' in value) || value['frontId'] === undefined) return false;
    if (!('url' in value) || value['url'] === undefined) return false;
    if (!('rank' in value) || value['rank'] === undefined) return false;
    if (!('rankDelta' in value) || value['rankDelta'] === undefined) return false;
    return true;
}

export function GlobalmapEventaccountratingsItemFromJSON(json: any): GlobalmapEventaccountratingsItem {
    return GlobalmapEventaccountratingsItemFromJSONTyped(json, false);
}

export function GlobalmapEventaccountratingsItemFromJSONTyped(json: any, ignoreDiscriminator: boolean): GlobalmapEventaccountratingsItem {
    if (json == null) {
        return json;
    }
    return {
        
        'eventId': json['event_id'],
        'accountId': json['account_id'],
        'clanId': json['clan_id'],
        'clanRank': json['clan_rank'],
        'battles': json['battles'],
        'battlesToAward': json['battles_to_award'],
        'awardLevel': json['award_level'],
        'updatedAt': json['updated_at'],
        'famePoints': json['fame_points'],
        'famePointsToImproveAward': json['fame_points_to_improve_award'],
        'frontId': json['front_id'],
        'url': json['url'],
        'rank': json['rank'],
        'rankDelta': json['rank_delta'],
    };
}

export function GlobalmapEventaccountratingsItemToJSON(json: any): GlobalmapEventaccountratingsItem {
    return GlobalmapEventaccountratingsItemToJSONTyped(json, false);
}

export function GlobalmapEventaccountratingsItemToJSONTyped(value?: GlobalmapEventaccountratingsItem | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'event_id': value['eventId'],
        'account_id': value['accountId'],
        'clan_id': value['clanId'],
        'clan_rank': value['clanRank'],
        'battles': value['battles'],
        'battles_to_award': value['battlesToAward'],
        'award_level': value['awardLevel'],
        'updated_at': value['updatedAt'],
        'fame_points': value['famePoints'],
        'fame_points_to_improve_award': value['famePointsToImproveAward'],
        'front_id': value['frontId'],
        'url': value['url'],
        'rank': value['rank'],
        'rank_delta': value['rankDelta'],
    };
}

